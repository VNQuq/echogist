...
| `Phase N/M … came back empty` | фаза вернулась без текста, то есть её кусок записи выпал бы из документа. Файл падает намеренно; повтор из сохранённого транскрипта переплатит только за эту фазу |
