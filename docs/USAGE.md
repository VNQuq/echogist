...
Навигация: **↑/↓** двигают, **Enter** выбирает, цифра выбирает строку по её номеру,
**← Back** — шаг назад. **ESC** и **Ctrl-C** выходят из приложения целиком — единственное
исключение описано в разделе про массовую конвертацию.
...
Когда нужно вытащить звук сразу из пачки записей. Шаг офлайновый и бесплатный: MP3 — весь
результат, ни транскрипции, ни резюме, ни облачных вызовов.

1. Выбери **Batch: videos → MP3**.
2. В системном окне отметь файлы: **Shift+клик** — диапазон, **Ctrl+клик** — поштучно,
   **Ctrl+A** — всё, что видно (фильтр окна уже отсекает не-медиа).
3. Если среди выбранного окажутся готовые MP3, EchoGist спросит один раз, перекодировать ли
   их тоже. По умолчанию **нет** — перекодировка MP3 только теряет качество, а диапазон,
   выделенный шифтом, легко цепляет соседний файл. Сжать MP3 осознанно можно через
   **Local file → Re-encode to a smaller MP3**.
4. Полоса прогресса считает **файлы**, а не проценты внутри файла: при нескольких
   одновременных конвертациях честного общего процента не существует.

Один битый файл не роняет пачку: он попадает в таблицу «Not converted» с причиной, остальные
досчитываются. В конце — строка «Converted N of M» и разовое открытие папки `output/audio`.

> **Ctrl-C здесь останавливает пачку, а не приложение.** Текущие конвертации гасятся за секунды
> (упрямые — принудительно через 5 с), очередь отменяется, уже готовые MP3 остаются на диске,
> отчёт печатается, и ты возвращаешься в меню. Это единственное место, где Ctrl-C не выходит из
> EchoGist. Второе нажатие не отменяет отчёт, третье закрывает приложение принудительно.
...
**Batch MP3 workers** — сколько файлов массовая конвертация обрабатывает одновременно.
`1` — строго по очереди. Больше четырёх обычно не ускоряет: ffmpeg на звуковой дорожке
упирается в чтение с диска, а не в процессор.
